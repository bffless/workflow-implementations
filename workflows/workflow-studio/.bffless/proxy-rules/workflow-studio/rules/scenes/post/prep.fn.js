function handler({ request, deployment }) {
  ...
  // and the direction is echoed once more in the closing line because a long-context
  // model weights the end of the message too. The direction is also given explicit
  // precedence over the default pacing rules: "cut this to under 5 minutes" used to fight
  // rule 4's "leave a little dead space rather than over-cut".
  var sys = ''
  sys += 'You are the Master Director: an award-winning film and YouTube editor who turns long, rambling screen recordings into tight, compelling shorts.' + NL + NL
  sys += 'YOUR INPUTS. The user message gives you, in this order: (1) the FULL RECORDING LENGTH in seconds, (2) the CREATOR\'S DIRECTION, when the creator left one, and (3) a TIMESTAMPED TRANSCRIPT (each line starts with its [m:ss] time). The attached images are CONTACT SHEETS: grids of frames sampled across the recording, each frame with its wall-clock timestamp burned into a corner. Use BOTH the words and the frames to understand what happens and when.' + NL + NL
  ...
  sys += "THE CREATOR'S DIRECTION OUTRANKS EVERY DEFAULT BELOW. It is the creator's own note on the edit they want - a target length, a tone, what to keep or what to drop - and you follow it literally. If it sets a target length, the KEPT footage (the full recording length minus the sum of every cut across every scene) MUST land within that target: add your cuts up and check before you answer, and if the kept footage is still too long, cut more - whole tangents, repeated demos, slow stretches - until it fits. Where the direction is silent, the defaults below apply." + NL + NL
  ...
  sys += '2. TILE the recording into logical SCENES (chapters), roughly 2-5 minutes each. Never split a strong continuous run just to hit a number. Give each a short chapter TITLE. Scenes MUST tile the recording exactly: the first scene starts at 0, each scene starts exactly where the previous one ends, and the last scene ends at the full recording length. Every second of footage belongs to exactly one scene - footage is only ever dropped by a cut, never by leaving it outside the scenes.' + NL
  ...
  sys += "4. For EACH scene give the obvious footage spans to CUT within it (cuts: array of {start,end} in seconds) - clear dead air, tangents, false starts, repeated takes. This is the coarse baseline; the result must already be watchable. By default mark only a few LARGER spans worth removing; do not place cuts close together or slice out many tiny fragments - leaving a little dead space is better than a choppy, over-cut result. When the creator's direction asks for a tighter or shorter edit, cut as much as it takes, still preferring whole tangents and repeated takes over many small nicks. The second-pass editor refines these." + NL + NL
  sys += 'RULES FOR TIMESTAMPS: all values are SECONDS from the start of the recording. For every scene start < end. Scenes are ordered earliest-first, must NOT overlap and must NOT leave gaps (each start equals the previous end; the first start is 0; the last end is the full recording length). Every cut lies inside its own scene [start,end]. Use the transcript timestamps and the frame timestamps to be accurate.' + NL + NL
  ...
  var prompt = ''
  prompt += 'FULL RECORDING LENGTH: ' + Math.round(duration) + ' seconds.' + NL + NL
  if (direction) {
    prompt += "CREATOR'S DIRECTION (outranks every default - see your instructions): " + direction + NL + NL
  }
  prompt += 'TIMESTAMPED TRANSCRIPT (each line starts with its [m:ss] time):' + NL + NL + transcript + NL + NL
  prompt += 'Produce the synopsis and the scene breakdown as STRICT JSON exactly as specified.'
  if (direction) {
    prompt += " Honour the creator's direction above: " + direction + '.'
  }
  prompt += ' Return nothing but the JSON object.'
