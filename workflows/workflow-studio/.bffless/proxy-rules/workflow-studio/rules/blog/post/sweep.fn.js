...
// the failure and carries on to the next step - so with `parse` thrown, neither finishOk
...
// branch has answered 400 there is no job row for finishStuck to write onto. `parse` is
...
function handler({ steps, stepErrors }) {
  ...
  var err = stepErrors && stepErrors.parse
  var detail = (err && (err.code || err.message)) ? ' (' + [err.code, err.message].filter(Boolean).join(': ') + ')' : ''
  return { stuck: true, message: "Job step 'parse' threw and wrote no output" + detail }
}
