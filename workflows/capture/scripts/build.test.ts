/**
 * What `vite.scripts.config.ts` must produce for each of the two entries: ONE
 ...
 * `vite build`) doesn't fail; the stager builds both before CI stages the bundle.
 */
import { describe, expect, it } from 'vitest'

/** The two entries the workflow YAML's `script` steps name (`src: scripts/<name>.js`). */
...
describe.each(SCRIPT_NAMES)('dist/scripts/%s.js', (name) => {
  ...
  it.skipIf(!fs.existsSync(file))('is one self-contained ES module with a default export', () => {
    ...
    // data, not syntax (same expression as scripts/stage.mjs).
