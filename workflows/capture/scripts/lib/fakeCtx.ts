 ...
 * out because the `scripts/*.test.ts` suites need it. Never imported by a script
 * module, so it is never bundled into a `dist/scripts/*.js` entry (only the named
 * entries under `scripts/` are built; `scripts/lib/` is support code).
 *
